...
def dashboard():
    ...
    try:
        ...
        # Fetch Users with Profiles (Only non-admins)
        cur.execute("""
            SELECT 
        ...
        # Convert sqlite3.Row or tuple to dictionary for easier access in template
        # Assuming sqlite3.Row is used, but being safe with manual conversion if needed
        user_list = []
...
def add_webinar():
    ...
    db = get_db()
    try:
        db.execute("INSERT INTO webinars (topic, description, date, time, created_at) VALUES (?, ?, ?, ?, ?)", 
                   (topic, description, date, time, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        db.commit()
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return jsonify({"success": True, "message": "Webinar scheduled successfully! 🛰️"})
...
def update_support_status(request_id):
    status = request.json.get('status') # 'pending' or 'completed'
    db = get_db()
    try:
        # Fetch request details before updating
        cur = db.cursor()
        cur.execute("SELECT name, email, message, status FROM support_requests WHERE id = ?", (request_id,))
        support_req = cur.fetchone()
        
        old_status = support_req[3] if support_req else None
        
        db.execute("UPDATE support_requests SET status = ? WHERE id = ?", (status, request_id))
        db.commit()
        ...
        # Send email if re-opened (changed from completed to pending)
